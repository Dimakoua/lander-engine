...
describe('ConfigParser', () => {
  ...
  it('should throw error if theme.json is missing', async () => {
    const mockFlow = { initialStep: 'step1' };
    (fs.pathExists as any).mockImplementation((p: string) => p.includes('flow.json'));
    (fs.readJson as any).mockImplementation((p: string) => {
      if (p.includes('flow.json')) return mockFlow;
      return null;
    });
    await expect(parser.loadCampaignBase('test-campaign')).rejects.toThrow('Missing mandatory theme.json');
  });

  it('should throw error if flow.json lacks initialStep', async () => {
    const mockFlow = {};
    (fs.pathExists as any).mockImplementation((p: string) => p.includes('flow.json'));
    (fs.readJson as any).mockImplementation((p: string) => {
      if (p.includes('flow.json')) return mockFlow;
    ...
    await expect(parser.loadCampaignBase('test-campaign')).rejects.toThrow('flow.json must have an \'initialStep\'');
  });

  it('should throw error if theme.json lacks colors', async () => {
    const mockFlow = { initialStep: 'step1' };
    const mockTheme = {};
    (fs.pathExists as any).mockImplementation((p: string) => p.includes('flow.json') || p.includes('theme.json'));
    (fs.readJson as any).mockImplementation((p: string) => {
      if (p.includes('flow.json')) return mockFlow;
      if (p.includes('theme.json')) return mockTheme;
      return null;
    });
    await expect(parser.loadCampaignBase('test-campaign')).rejects.toThrow('theme.json must have a \'colors\' object');
  });

  it('should throw error if step lacks sections', async () => {
    const mockFlow = { initialStep: 'step1' };
    const mockTheme = { colors: {} };
    const mockStep = {};
    (fs.pathExists as any).mockImplementation((p: string) => p.includes('flow.json') || p.includes('theme.json') || p.includes('step1.json'));
    (fs.readJson as any).mockImplementation((p: string) => {
      if (p.includes('flow.json')) return mockFlow;
      if (p.includes('theme.json')) return mockTheme;
      if (p.includes('step1.json')) return mockStep;
    ...
    (glob as any).mockResolvedValue(['test-campaign/steps/step1.json']);
    await expect(parser.loadCampaignBase('test-campaign')).rejects.toThrow('must have a \'sections\' array');
  });

  it('should throw error if campaign has no steps', async () => {
    const mockFlow = { initialStep: 'step1' };
    const mockTheme = { colors: {} };
    (fs.pathExists as any).mockImplementation((p: string) => p.includes('flow.json') || p.includes('theme.json'));
    (fs.readJson as any).mockImplementation((p: string) => {
    ...
    (glob as any).mockResolvedValue([]);
    await expect(parser.loadCampaignBase('test-campaign')).rejects.toThrow('must have at least one step');
  });

  it('should handle malformed JSON', async () => {
    (fs.pathExists as any).mockResolvedValue(true);
    (fs.readJson as any).mockRejectedValue(new Error('Unexpected token'));
    await expect((parser as any).readJson('test.json')).rejects.toThrow('Failed to parse JSON at test.json: Unexpected token');
  });

  it('should ignore missing parts and not crash when interpolating', async () => {
    const state = {};
    const result = interpolateVariables(undefined as any, state);
    expect(result).toBeUndefined();

    const result2 = interpolateVariables(null as any, state);
