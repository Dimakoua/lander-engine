...
`;
  ...
  private async generateDomainRouting() {
    ...
    for (const [domain, routeConfig] of Object.entries(routingConfig)) {
      ...
      // Normalize base path to start with / and not end with /
      ...
      if (basePath === '/') {
        hasRootBasePath = true;
      }

      normalizedRouting[domain] = { campaign: campaignId, basePath, defaultStep };
      ...
      try {
        ...
        if (basePath === '/') {
          targetPath = `/${initialStep}`;
        } else if (basePath) {
          targetPath = `${basePath}/${initialStep}`;
        }

        domainPaths[domain] = targetPath;
      } catch {
        console.warn(
          `routing.config.js: could not read flow.json for campaign "${campaignId}" (domain: ${domain}), falling back to ${basePath || `/${campaignId}`}`
        );
        domainPaths[domain] = basePath || `/${campaignId}`;
      }
    }
    ...
    // Only generate index.astro redirector if no domain claims the root (/) path,
    // otherwise [...slug].astro needs to handle it directly.
    if (!hasRootBasePath) {
...
`;
    ...
    for (const [domain, targetPath] of Object.entries(domainPaths)) {
      // Netlify redirect from root for specific host
