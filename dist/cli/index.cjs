...
var WorkspaceGenerator = class {
  ...
  async generate() {
    ...
    if (await _fsextra2.default.pathExists(templateDir)) {
      ...
      try {
        ...
        const configPath = _path2.default.join(this.workspaceDir, "astro.config.mjs");
        let configContent = await _fsextra2.default.readFile(configPath, "utf8");
        configContent = configContent.replace("{{ASTRO_COMPRESSOR_PATH}}", compressorPath);
        await _fsextra2.default.writeFile(configPath, configContent);
      } catch (copyErr) {
  ...
  resolvePackage(packageName) {
    try {
      const packagePath = _path2.default.join(this.config.engineRoot, "node_modules", packageName);
      if (_fsextra2.default.existsSync(packagePath)) {
        return _url.pathToFileURL.call(void 0, packagePath).toString();
      }
      return packageName;
    } catch (e2) {
      return packageName;
...
var Builder = class {
  ...
  async preview(port = 4321) {
    ...
    const sirv = (await Promise.resolve().then(() => _interopRequireWildcard(require("sirv")))).default;
    const http = await Promise.resolve().then(() => _interopRequireWildcard(require("http")));
